...
- [ ] T1 冻结 20 页路由与案件状态到用户动作的映射；文件：`utils/case-router.js`、本功能 `context.md`；验收：覆盖 created/responded/tried/closed、pact、review、hasB、inbox，并把 `case-detail` 作为 P0 中枢；验证：本地 Mock 和至少一份云端投影样本逐条核对。
- [ ] T2 建立页面契约表；文件：本功能 `context.md`、`verification.md`、设计 Figma UI Contract；验收：20 个页面均记录 route、entry、actor、privacy、async、failure、back、saved、qa-selector、priority。
...
- [ ] T6 改造卷宗列表与状态摘要；文件：`pages/history/history.js`、`pages/history/history.wxml`、`pages/history/history.wxss`；验收：历史入口统一进入 `case-detail`；验证：未发传票、等待应诉、已判决、待复盘、已复盘、约定冲突。
- [ ] T6.1 新增 Case Detail P0 页面；文件：`app.json`、`pages/case-detail/`、`utils/case-router.js`、`utils/navigation.js`；验收：History/Home/inbox 携带 `docId` 进入详情，详情显示案件状态、双方进度和唯一主操作，并能进入正确目标页；验证：案件不存在、无权限、字段缺失、重试和栈为空回 Home。
...
- [ ] T11.1 完成代码页面 ↔ Figma 页面映射；文件：`app.json`、本功能 `page-contracts.md`、`09-UI设计需求.md`、独立 UI Figma；验收：20 个注册页面均有对应 Figma 页面、route、priority 和状态入口，Home 三态和 Case Detail 单独标记。
...
- [ ] T14 更新项目技术文档；文件：`docs/technical-architecture.md`、`docs/status.md`；验收：补充 Home View Model、20 页契约、状态组件、路由工具和数据流；验证：链接和 Mermaid/代码块可读。
...
- 品牌 Figma 基础已存在；独立 UI Figma 的页面映射、UI Foundation、20 页 Screens、系统态和 Prototype 尚未完整交付。
