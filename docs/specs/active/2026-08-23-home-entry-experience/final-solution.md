...
- [20 页 UI Contract](page-contracts.md)

## 一、评审后的总判断

当前小程序已有 19 个业务页面和完整主流程；方案新增 1 个 P0 `Case Detail`，形成 20 页业务路由。下一步重点是把以下四件事做成可执行系统：
...
2. Home 是关系首页，History 是全部案件，Case Detail 是单案状态与下一步，Profile 是关系模式与隐私设置。
...
- `case-detail` 的案件状态、双方进度、下一步和恢复态
...
```text
Profile：查看当前账户余额、额度来源、冻结/可用状态
AI 调用前：展示本次预计消耗、调用后预计余额、余额不足时的恢复路径
AI 调用后：展示实际扣费、释放或失败原因
后台：按 OPENID 查询、发放、扣回、冻结、解冻和审计
```

- 黑客松运行模式固定为 `BILLING_MODE=enforced`，以真实账本扣减为准。
- 额度不是支付余额；本阶段不接支付、不开放用户充值。
- 任何价格和余额判断以服务端为准，前端只能展示预估，不得自行决定是否扣费。
- Profile 的积分卡和 AI 调用前预估属于 P0 交付；真实 CloudBase 环境、集合/索引、管理员白名单和测试账号是运行门槛。
